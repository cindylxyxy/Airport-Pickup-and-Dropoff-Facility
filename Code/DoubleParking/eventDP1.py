...
class system():
	...
	def prepare_pulling_out(self, curr_vehicle):
		...
		# if a vehicle is entering the spot right in front of the current vehicle
		# the current vehicle cannot maneuver into the through lane until the enter maneuver is at least partially done
		# if (stop <= self.N - 1) and (self.inservice[1][stop] is not None) and (self.inservice[1][stop].status == 1.5):
		# 	car = self.inservice[1][stop]
		# 	if curr_vehicle.idx == VEHICLE_IDX and car.plin_end - min(meanDRIV, max(0, car.plin_time - .5)) > req_time:
		# 		delay_reason = 2
		# 		delay_status = 1.5

	...
	def __gt__(self, other):
		if float(self.time) == float(other.time):
			if not spmatch and angle == 0 and mode =='long':
				if self.typ == 'start_service' and self.vehicle.status == 2 and self.vehicle.nex is not None and self.vehicle.nex.status == 5 and self.vehicle.nex.stop + 1 == self.vehicle.stop:
					assert self.vehicle.curr_loc == self.vehicle.nex.curr_loc
					return False
				if other.typ == 'start_service' and other.vehicle.status == 2 and other.vehicle.nex is not None and other.vehicle.nex.status == 5 and other.vehicle.nex.stop + 1 == other.vehicle.stop:

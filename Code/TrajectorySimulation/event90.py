...
class system90(system):
	...
	def start_pulling_in(self, curr_vehicle):
		...
		# if (curr_vehicle.prev is not None) and (curr_vehicle.prev.status not in [2, 5]) and (curr_vehicle.prev.curr_loc <= curr_vehicle.dest_to_stop + 1.5 * CAR_LENGTH):
	...
	def enter_system(self, curr_vehicle, debug_idx = None):
		...
		# for idx in range(1, 1 + nUnit):
		# 	if self.inservice[idx - 1] is not None and self.inservice[idx - 1].status == 4:
		# 		req_time = max( req_time, self.inservice[idx - 1].end_time)
		# 		assert (req_time > self.curr)

		# if side == 'double':
		# 	for idx in range(1 + nUnit, 1 + 2 * nUnit):
		#  		if self.inservice[idx - 1] is not None and self.inservice[idx - 1].status == 4:
		# 			req_time = max( req_time, self.inservice[idx - 1].end_time) 
		# 			assert (req_time > self.curr)
	...
	def check_lane_90(self, curr_vehicle, first_attempt, delay_reason = None, delay_status = None, delay_speed = None):
		...
		while car != None:
			...
			if car.stop > stop + dgap and car.status == 2:
				assert car.plin_start <= self.curr
			...
			elif car.curr_loc >= (stop - 1) * LOT_LENGTH + 2 * CAR_LENGTH - SMALL_INTERVAL:
				prev = car
				...
				if car_time < meanPOUT:
					delayed = True
					car_time = car.calc_time( min(car.dest_to_stop, (stop - 1) * LOT_LENGTH + 2 * CAR_LENGTH) )
					assert car_time > self.curr
				...
				if not (car.stop < stop - dgap or car.plin_end == self.curr):
					import pdb; pdb.set_trace()
				...
				if car_time < meanPOUT:
					...
					if car.status == 1:
						...
						if car_time <= self.curr and car_time > self.curr - SMALL_INTERVAL:
							car_time += SMALL_INTERVAL

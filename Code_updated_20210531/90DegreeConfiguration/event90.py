...
class system90(system):
	...
	def prepare_pulling_out(self, curr_vehicle):
		...
		# if self.curr >= 4764. and curr_vehicle.stop_idx == 1:
		# 	import pdb; pdb.set_trace()

		# if curr_vehicle.idx == 446:
		# 	import pdb; pdb.set_trace()
	...
	def enter_system(self, curr_vehicle, debug_idx = None):
		...
		for j in sorted(self.waiting, reverse = True):
			...
			if (spot2blk(J) < car.block_idx - 1) or (car.status == 6):
				...
				assert car.stop_idx != j
				assert car.block_idx >= 2
				if not car.curr_loc >= CAR_LENGTH + curr_vehicle.curr_loc - SMALL_INTERVAL:
					import pdb; pdb.set_trace()
	...
	def check_lane_90(self, curr_vehicle, first_attempt, delay_reason = None, delay_status = None, delay_speed = None, curr_time = None):
		...
		# if self.curr >= 1155720. and curr_vehicle.stop_idx == 12:
		# 	import pdb; pdb.set_trace()
		...
		while car != None:
			...
			if car.idx == curr_vehicle.idx:
				...
				if car.stop == stop and est_pout_start < car.plin_end - 10 * SMALL_INTERVAL:
					...
					if car.block_idx == block_idx + 1 and (car.stop > stop + dgap) and curr_time < car.plin_end - meanPOUT:
						...
						if spmatch and est_pout_end + (car.block_idx - 1 - block_idx) * CAR_LENGTH / rateDRIV < max(curr_time, car.plin_start + meanPLIN) - 2e-05:
							...
							if not car.plin_start + meanPLIN > est_pout_start + meanPOUT + (car.block_idx - 1 - block_idx) * CAR_LENGTH / rateDRIV:
								import pdb; pdb.set_trace()
							else:	
								delayed = True
								req_time = max( req_time, meanPLIN + car.plin_start - (meanPOUT + (car.block_idx - 1 - block_idx) * CAR_LENGTH / rateDRIV))
							...
							if not car.plin_start + meanPLIN > est_pout_start + meanPOUT + (car.stop - 1 - stop) * LOT_LENGTH / rateDRIV:
								...
								delayed = True
								req_time = max( req_time, meanPLIN + car.plin_start - (meanPOUT + (car.stop - 1 - stop) * LOT_LENGTH / rateDRIV))	
					else:
						prev = car				

			elif car.status == 5 and car.stop_idx in out_range(curr_vehicle.stop_idx, self.N) and car.pout_end - curr_time > SMALL_INTERVAL:
				...
				if car.pout_start == car.serv_end:
				...
				if car.block_idx == block_idx + 1 and car.status == 1:
					...
					car_time = est_pout_end + (car.block_idx - block_idx - 1) * CAR_LENGTH / rateDRIV 
					if car_time < car.end_time + meanPLIN - 2e-05:
						if not car.end_time + meanPLIN > est_pout_start + meanPOUT + (car.block_idx - block_idx - 1) * CAR_LENGTH / rateDRIV:
							import pdb; pdb.set_trace()
						else:
							delayed = True
							req_time = max( req_time, car.end_time + meanPLIN - meanPOUT - (car.block_idx - block_idx - 1) * CAR_LENGTH / rateDRIV )
					else:
						prev = car
				...
				if (control == 'full' or ptype == 0) and car.stop > (stop + dgap) and car.status == 1:
					...
					if car_time < car.end_time + meanPLIN - 2e-05:
						if not car.end_time + meanPLIN > est_pout_start + meanPOUT + (car.stop - 1 - stop - dgap) * LOT_LENGTH / rateDRIV:
							...
							delayed = True
							req_time = max( req_time, car.end_time + meanPLIN - meanPOUT - (car.stop - 1 - stop - dgap) * LOT_LENGTH / rateDRIV )
					else:
						prev = car
				else:
					prev = car

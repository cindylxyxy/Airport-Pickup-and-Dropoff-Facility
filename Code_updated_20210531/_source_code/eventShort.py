########################################################################################################################################################
# Last updated: May 9, 2021 by Xinyu Liu
...
# This file deals specifically with 0-degree configurations with short spots.
	...
	def check_lane_zero_short(self, curr_vehicle, first_attempt, delay_reason = None, delay_status = None, delay_speed = None, curr_time = None):
		...
		while car != None:
			...
			if car.idx == curr_vehicle.idx:
				...
				elif (control == 'full' or ptype == 0) and est_pout_end + (car.stop - 1 - stop) * LOT_LENGTH / rateDRIV < max(curr_time, car.plin_start + meanPLIN) - SMALL_INTERVAL:
					# import pdb; pdb.set_trace()
				...
				# elif car.stop > stop and car.status == 1:
					...
					if car_time < car.end_time + meanPLIN - 1e-05:
						if not car.end_time + meanPLIN > est_pout_start + meanPOUT + (car.stop - stop - 1) * LOT_LENGTH / rateDRIV:
							import pdb; pdb.set_trace()
						else:
							delayed = True
							req_time = max( req_time, car.end_time + meanPLIN - meanPOUT - (car.stop - stop - 1) * LOT_LENGTH / rateDRIV )
					else:
						prev = car
				...
				elif car.stop >= stop - dgap and est_pout_start + meanDRIV < car.plin_end - 10 * SMALL_INTERVAL:
					stopped = False
					delayed = True
				...
				if stopped and car.prev.status == 1 and car.prev.stop_idx == curr_vehicle.stop_idx:
					...
					if not (car.prev.stop - 1) * LOT_LENGTH >= car.curr_loc - 1e-04:
						import pdb; pdb.set_trace()

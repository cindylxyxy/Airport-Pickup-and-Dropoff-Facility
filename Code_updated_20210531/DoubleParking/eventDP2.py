...
# This file assumes Double Parking Configuration with spot assignment rule - alternative 2. 
...
class vehicle():
	...
	def update_loc(self):
		...
		if cp.data.t > self.sys.curr:
			print ('Line 1857: current time not accounted for in the trajectory!!!')
			import pdb; pdb.set_trace()

		if self.end_time < self.sys.curr - SMALL_INTERVAL:
			print ('Line 1861: self.end_time, self.sys.curr', self.end_time, self.sys.curr)
			import pdb; pdb.set_trace()
		...
		while cp.nex.data.t <= self.sys.curr:
			...
			if cp.nex == None:
				if np.abs(self.end_time - self.sys.curr) > SMALL_INTERVAL:
					print ('Line 1868: it should be small', np.abs(self.end_time - self.sys.curr))
					import pdb; pdb.set_trace()
				self.curr_loc = cp.data.x
				return
	...
	def update_traj(self):
		...
		if self.prev == None:
			try:
				...
				print ('Line 1946: this car does not have a prev while self.sys.head is not itself!!!')
				import pdb; pdb.set_trace()
		...
		if (cp.nex is None):
			print ('line 1989: testing if an option is possible')
			import pdb; pdb.set_trace()

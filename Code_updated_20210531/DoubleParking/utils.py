...
def Unif2(rate, seed = None):
	# returning rate
	curr = random.Random(seed)
	while True:
		yield curr.uniform(0.5 * rate, 1.5 * rate)
		...
		if float(self.time) == float(other.time):
			if (self.typ == other.typ):
				if (self.vehicle.idx is None) | (other.vehicle.idx is None):
					...
					return False
				assert self.vehicle.stop is not None and other.vehicle.stop is not None
				if (self.vehicle.stop == other.vehicle.stop):
					if not self.typ in ['start_pulling_in', 'start_second_enter', 'leave_system']:
						import pdb;pdb.set_trace()
					return (self.vehicle.idx > other.vehicle.idx)
				return (self.vehicle.stop < other.vehicle.stop)
			return (event_priority[self.typ] > event_priority[other.typ])
		return (float(self.time) > float(other.time))

	def __lt__(self, other):
		if float(self.time) == float(other.time):
			if self.typ in ['start_pulling_in', 'start_service'] and self.vehicle.status == 1.5 and self.vehicle.nex is not None and self.vehicle.nex.status == 5 and self.vehicle.nex.stop + 1 == self.vehicle.stop:
				assert self.vehicle.curr_loc == self.vehicle.nex.curr_loc
				return True
			if other.typ in ['start_pulling_in', 'start_service'] and other.vehicle.status == 1.5 and other.vehicle.nex is not None and other.vehicle.nex.status == 5 and other.vehicle.nex.stop + 1 == other.vehicle.stop:
				assert other.vehicle.curr_loc == other.vehicle.nex.curr_loc
				return False
			if self.typ == other.typ:
				if (self.vehicle.idx is None) | (other.vehicle.idx is None):
					assert (self.vehicle.idx is None) and (other.vehicle.idx is None)
